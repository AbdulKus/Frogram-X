...
public class NavigationController implements Future<View>, ThemeChangeListener, Lang.Listener, SystemBackEventListener {
 ...
 /* @Override
  ...
  public void setLayerType (final int type, final ViewController<?> left, final ViewController<?> right) {
    ...
    boolean liveLeft = left != null && (left.hasFloatingHeader() ||
      left instanceof org.thunderdog.challegram.ui.MessagesController && !left.allowLayerTypeChanges());
    boolean liveRight = right != null && (right.hasFloatingHeader() ||
      right instanceof org.thunderdog.challegram.ui.MessagesController && !right.allowLayerTypeChanges());
    boolean livePages = liveLeft || liveRight;
    ...
    if (left != null && (left.allowLayerTypeChanges() || liveLeft)) {
      left.getValue().setLayerType(livePages ? View.LAYER_TYPE_NONE : type, null);
    }
    if (right != null && (right.allowLayerTypeChanges() || liveRight)) {
